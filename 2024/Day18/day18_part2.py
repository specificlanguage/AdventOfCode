...
# We already simulated/verified up to LIMIT bytes in the first part, so we can at least skip those
for i in range(LIMIT, len(lines)):
    r, c = lines[i]
    grid[r][c] = "#"
    # Postnote, when submitting answer this make sure the format is (c, r)
    if findExit() == -1:
        print(i)
        break